...
from src.youtube import SongQueue, YTDLSource, YTDLError, Song
...
class VoiceError(Exception):
    def __init__(self, message="VoiceError"):
        print(message)
...
class Music(commands.Cog):
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Вы не подключены к голосовому каналу, мастер. И не указали куда подключаться.')
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send('Нечего останавливать, мастер.')
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Музыка не играет, мастер')
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        ...
        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('Уже подключен с голосовому каналу.')


if __name__ == "__main__":
    client = commands.Bot('!', description='music bot')
    ...
    async def on_ready():
        ...
        await client.change_presence(status=discord.Status.idle, activity=Activity(name="за музыкой.",type=ActivityType.watching))
